...
import { apiAdminCreatePost } from "../../../../services/admin.posts.api";
...
export default function AdminPostNew() {
  ...
  const pickImages = useCallback(async () => {
    try {
      ...
      const next: PickedImage[] = res.assets.map((a) => ({
        localUri: a.uri,
        uploading: false,
      }));
  ...
  const uploadOne = useCallback(
    async (idx: number) => {
      ...
      try {
        setImages((prev) =>
          prev.map((it, i) => (i === idx ? { ...it, uploading: true } : it))
        );

        // filename + mime best effort
        ...
        setImages((prev) =>
          prev.map((it, i) => (i === idx ? { ...it, uploading: false } : it))
        );
  ...
  const uploadAllMissing = useCallback(async () => {
    // upload only those not uploaded yet
  ...
  const onSave = useCallback(async () => {
    ...
    if (uploadedUrls.length < 1)
      return Alert.alert("Validation", "Ajoute au moins 1 image.");

    setSaving(true);
    try {
      const res = await apiAdminCreatePost({
        text: text.trim(),
        images: uploadedUrls,
        isPublished: false,
      });

      Alert.alert("OK", "Publication créée.");
      router.replace(`/settings/admin/posts/${res.post.id}`);
    } catch (e: any) {
      Alert.alert("Erreur", e?.message ?? "Impossible de créer la publication.");
    } finally {
      setSaving(false);
    }
  }, [text, uploadedUrls]);
    ...
    >
      ...
      >
        ...
        <Text style={{ color: "rgba(234,240,255,0.72)", marginTop: 6 }}>
          Minimum 1 image. Puis tu peux publier depuis la page détail.
        </Text>
