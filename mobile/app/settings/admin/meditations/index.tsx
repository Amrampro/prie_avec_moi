...
export default function AdminMeditationsScreen() {
  ...
  const load = useCallback(async () => {
    const res = await apiAdminListMeditations();
    ...
    >
      ...
      >
        ...
        <View style={{ marginTop: 14 }}>
          <Pressable
            onPress={() => router.push("/settings/admin/meditations/new")}
            style={{
              paddingVertical: 12,
              borderRadius: 18,
          ...
          {loading ? (
            ...
            meditations.map((m) => (
              ...
              >
                ...
                >
                  Slug: {m.slug} • {m.isPublished ? "Publié" : "Brouillon"}
                </Text>
                ...
                >
                  ...
                  <Pressable
                    onPress={async () => {
                      try {
                        if (m.isPublished) await apiAdminUnpublishMeditation(m.id);
                        else await apiAdminPublishMeditation(m.id);
                        await load();
                      } catch (e: any) {
                  ...
                  >
                    <Ionicons
                      name={m.isPublished ? "eye-off-outline" : "eye-outline"}
                      size={16}
                      color="#EAF0FF"
                    />
                    <Text style={{ color: "#EAF0FF", fontWeight: "900" }}>
                  ...
                  {/* ✅ Voir (Edit) */}
                  <Pressable
                    onPress={() => router.push(`/settings/admin/meditations/${m.id}`)}
                    style={{
                      width: 54,
                      paddingVertical: 10,
                      borderRadius: 14,
                    ...
                    onPress={() => {
                      Alert.alert("Supprimer", "Supprimer cette méditation ?", [
                        { text: "Annuler", style: "cancel" },
                        {
                          ...
                          onPress: async () => {
                            try {
                              await apiAdminDeleteMeditation(m.id);
                              await load();
                            } catch (e: any) {
                              Alert.alert(
                                "Erreur",
