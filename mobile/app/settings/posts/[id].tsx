// mobile/app/settings/posts/[id].tsx
...
export default function MyPostEdit() {
  ...
  const load = useCallback(async () => {
    if (!id) return;
    const res = await apiMyGetPost(id);
    ...
    useCallback(() => {
      ...
      (async () => {
        try {
          ...
          if (mounted) Alert.alert("Erreur", e?.message ?? "Impossible de charger le post.");
        } finally {
  ...
  const uploadOne = useCallback(
    async (idx: number) => {
      ...
      if (item.remoteUrl && item.localUri === item.remoteUrl) return;
  ...
  const onSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const res = await apiMyUpdatePost(id, {
        text: text.trim(),
        images: uploadedUrls,
      });

      setPost(res.post);
      Alert.alert("OK", "Post mis à jour.");
      router.replace("/settings/posts");
    } catch (e: any) {
  ...
  const onTogglePublish = useCallback(async () => {
    if (!id) return;
    try {
      if (isPublished) await apiMyUnpublishPost(id);
      else await apiMyPublishPost(id);
      await load();
    } catch (e: any) {
  ...
  const onDelete = useCallback(() => {
    ...
    Alert.alert("Supprimer", "Supprimer ce post ?", [
      { text: "Annuler", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            await apiMyDeletePost(id);
            Alert.alert("OK", "Post supprimé.");
            router.replace("/settings/posts");
          } catch (e: any) {
  ...
  return (
    ...
    >
      ...
      >
        <Text style={{ color: "#EAF0FF", fontSize: 22, fontWeight: "900" }}>Mon post</Text>
          ...
          <>
            ...
            <View style={{ marginTop: 14, flexDirection: "row", gap: 10 }}>
              ...
              >
                ...
                <Text style={{ color: "#EAF0FF", fontWeight: "900" }}>Ajouter images</Text>
              </Pressable>
            ...
            <View style={{ marginTop: 12, gap: 10 }}>
              {images.length === 0 ? (
                ...
                >
                  <Text style={{ color: "rgba(234,240,255,0.72)" }}>Aucune image (minimum 1).</Text>
                </View>
              ) : (
                images.map((img, idx) => {
            ...
            <View style={{ marginTop: 14, flexDirection: "row", gap: 10 }}>
              ...
              >
                <Ionicons name={isPublished ? "eye-off-outline" : "eye-outline"} size={18} color="#EAF0FF" />
                <Text style={{ color: "#EAF0FF", fontWeight: "900" }}>{isPublished ? "Dépublier" : "Publier"}</Text>
              </Pressable>
